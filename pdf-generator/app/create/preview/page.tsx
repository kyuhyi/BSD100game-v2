...
const BookScene = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-full aspect-square bg-secondary rounded-xl animate-pulse" /> }
);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">eBook의 최종 모습을 확인하고 PDF로 내보내세요.</p>
          </div>
          <ExportButton />
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { label: '제목', value: book.title || '미설정', color: 'text-primary' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="space-y-6">
            <div>
              ...
              <CoverPreview title={book.title} subtitle={book.subtitle} authorName={book.author.name} cover={book.cover} className="max-w-xs" />
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-3">3D 목업</h3>
              <BookScene colorScheme={book.cover.colorScheme} title={book.title} className="max-w-xs" />
            </div>
          </div>
          <div className="lg:col-span-2"><BookPreview /></div>
        </div>

        <div className="mt-12 flex justify-between items-center">
          <Button variant="ghost" asChild><Link href="/create/author"><ArrowLeft className="size-4" /> 저자 프로필</Link></Button>
          <ExportButton />
        </div>
      </motion.div>
    </div>
